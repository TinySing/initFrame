...
/** 添加请求拦截器 **/  //发请求前做的一些处理，数据转化，配置请求头，设置token,设置loading等，根据需求去添加
...
request.interceptors.response.use(response => {
  ...
  if (error.response) {
    ...
    // if (error.response.status === 401) {    // token或者登陆失效情况下跳转到登录页面，根据实际情况
    //   router.push({
    //     path: `/login`
    //   })
    // }
...
/* 统一封装get请求 */   // 不用  留着以后封装需要设置content-type上传文件等方式
